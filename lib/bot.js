...
function generateMsg(configUrl, issue, emoji) {
	return '<' + configUrl + issue.key.toUpperCase() + '|*' + issue.key.toUpperCase() + '*';
}
    ...
    jiraApi.searchJira(searchString, optional, function (error, body) {
      ...
      if (body && body.total > 0) {
        ...
        _.each(body.issues, function (item) {
          ...
          fullMsg += generateMsg(configUrl, item, self.config.emoji);
...
Bot.prototype.run = function () {
  ...
  var prjNames = "";
  var projects = self.config.projects.split(' ');
  console.log(projects);
  _.each(projects, function (prj, index, list) {
    pattern += prj;
    prjNames += prj;
          ...
          try {
            if (jiraApi && jiraApi != "NONE") {
              (function (def, jira) {
                ...
                jiraApi.findIssue(jira, function (error, issue) {
                  ...
                  if (issue && self.config.post == true) {
                    var configUrl = getBaseUrl(prjName, self.config);
                    msg = generateMsg(configUrl, issue, self.config.emoji);
              ...
              (function (def, jira) {
                requests.push(def.promise);
                var configUrl = getBaseUrl(prjName, self.config);
                var msg = generateMsg(configUrl, {key: jira}, self.config.emoji);
                ...
                var issuekey = {key: jira}.key;
				var url = self.config.baseurl + "rest/api/2/issue/"+issuekey+"?fields=summary&os_username="+self.config.username+"&os_password=" + self.config.password;
				console.log(url);
                request(url, function (error, response, body) {
	                if (!error && response.statusCode == 200) {
                		var body_json = JSON.parse(body);
						var title = escapeHtml(body_json.fields['summary']);
						msg += " " + title + '>';
					} else {
						msg += ">";
					}
