...
function generateMsg(configUrl, issue, emoji, self) {
	...
	return emoji + ' <' + configUrl + issue.key + '|*' + issue.key + '* ' + title + '>';
}
    ...
    jiraApi.searchJira(searchString, optional, function (error, body) {
      ...
      if (body && body.total > 0) {
        ...
        _.each(body.issues, function (item) {
          ...
          fullMsg += generateMsg(configUrl, item, self.config.emoji, self);
          ...
          try {
            if (jiraApi && jiraApi != "NONE") {
              (function (def, jira) {
                ...
                jiraApi.findIssue(jira, function (error, issue) {
                  ...
                  if (issue && self.config.post == true) {
                    var configUrl = getBaseUrl(prjName, self.config);
                    msg = generateMsg(configUrl, issue, self.config.emoji, self);
              ...
              (function (def, jira) {
                requests.push(def.promise);
                var configUrl = getBaseUrl(prjName, self.config);
                var msg = generateMsg(configUrl, {key: jira}, self.config.emoji, self);
